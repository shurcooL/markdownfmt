// Package debug provides a debug renderer. It prints the method names and parameters as they're called.
package debug
...
func (_ *debugRenderer) Image(out *bytes.Buffer, link []byte, title []byte, alt []byte) {
	fmt.Println(gist6418290.GetParentFuncArgsAsString(string(link), string(title), string(alt)))
}
